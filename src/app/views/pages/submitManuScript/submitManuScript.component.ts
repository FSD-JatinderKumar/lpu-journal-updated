...
//  For Editor  Logic
  ...
  actionList: any[] = [{id:'1',name:'Author'},{id:'2',name:'Reviewer'}];
  selectedAction: string = '';
// Function to handle action selection
TakeActionAs() {
  if (this.selectedAction === 'Reviewer') {
    this.isReviewer = true;  // Set reviewer to true when Reviewer is selected
    this.isAuthor = false;   // Ensure author is false
  } else if (this.selectedAction === 'Author') {
    this.isAuthor = true;    // Set author to true when Author is selected
    this.isReviewer = false; // Ensure reviewer is false
  }
}
  ...
  onTakeAction2(journalId: string) {
    console.log('Taking action on journal:', journalId);
  }
  ...
  submitReviewerForm() {
    ...
    let modal = bootstrap.Modal.getInstance(document.getElementById('assignReviewerModal'));
    modal.hide(); // Close modal after form submission
  }
}
